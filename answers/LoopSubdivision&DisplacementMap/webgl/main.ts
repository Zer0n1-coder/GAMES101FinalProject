...
let camera = new Camera(vec3.fromValues(0.0, 0.0, 20));
...
export function main(): void {
    ...
    let modeling = new Model('./monsterfrog', 'monsterfrog_mapped.obj', modelingShader);
    let normalTexture = loadTexture('monsterfrog-n', gl.RGB);
    let displacementTexture = loadTexture('monsterfrog-d', gl.RGB);

    modelingShader.setInt('texture_normal', 0);
    modelingShader.setInt('displacementTexture', 1);

    gl.enable(gl.DEPTH_TEST);

    function render() {
        let currentFrame = (new Date).getTime() / 100;
        ...
        modelingShader.setVec3("viewPos", camera.Position[0], camera.Position[1], camera.Position[2]);
