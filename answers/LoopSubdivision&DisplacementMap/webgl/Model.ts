...
export class Model {
    ...
    private processGroup(objGroup: ObjGroup, mtlObj: MtlObj) {
        ...
        let verticeNum = objGroup.ev.length / 3;
        for (let i = 0; i < verticeNum; ++i) {
            let vertex = new Vertex;

            //顶点和法线
            let index = i * 3;
            vertex.Position[0] = objGroup.ev[index];
            vertex.Position[1] = objGroup.ev[index + 1];
            vertex.Position[2] = objGroup.ev[index + 2];
            ...
            //纹理坐标
            index = i * 2;
            vertex.TexCoords[0] = objGroup.vt[index];
            vertex.TexCoords[1] = objGroup.vt[index + 1];
        ...
        for (let i = 0; i < objGroup.f.length; ++i)
        {
            ...
            indices.push(face.vi[3]);
            indices.push(face.vi[4]);
            indices.push(face.vi[5]);
        }
        ...
        //    textures = this.textures_loaded.get(objGroup.mtl);
        //}
        let mesh = new Mesh(vertices, indices);
